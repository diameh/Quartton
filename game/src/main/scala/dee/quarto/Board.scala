...
//    }
  ...
  def allRowsFigures(filledPositions: Map[Pos, Figure], lastAdded: Pos) = {
    filledPositions.collect {
      case (Pos(lastAdded.row, _), figure) => figure
    }
  }

  def allColumnsFigures(filledPositions: Map[Pos, Figure], lastAdded: Pos) = {
    filledPositions.collect {
  ...
  def calculateScore(filledPositions: Map[Pos, Figure], lastAdded: Pos) = {
    score(allRowsFigures(filledPositions, lastAdded).toSet) + score(allColumnsFigures(filledPositions, lastAdded).toSet)
  }

  def score(figures: Set[Figure]): Int = {
    if (figures.size == 4) 8 - getUniqueFeatures(figures).size //a point for every feature in common
    else 0
  }
             ...
             )
{
  ...
  require(figureToMove.isEmpty && filledPositions.size == 16 || !unusedFigures.contains(figureToMove.get),
    s"Chosen figure must not be in the set of unused figures ${figureToMove.get} ${filledPositions.size}")
  ...
  def makeMove(pos: Pos, nextFigure: Option[Figure]): MoveResult = {
    nextFigure match {
      ...
      case Some(figure) =>  if (!unusedFigures.contains(figure)) FailedMove("Figure already used")
                            ...
                            else completeASuccessfulMove(pos, nextFigure, unusedFigures - figure)
    }
  }
